...
public class SimpleTLMutation2 implements MutationOperator<TimeListSolution>
{
	...
	/**  Constructor */
	public SimpleTLMutation2(double probability, double impact) throws JMetalException{
	...
	private void doMutation(TimeListSolution solution) {
		...
		for(int c=0; c<changes; c++) {
			// calculate sum of the number of arrival times
			// if this solution is mutated,the size of variable can be change. 
			range = 0;
			for(TimeList gene:solution.getVariables()) {
				range += gene.size();
			}
			...
			// ---------------------------------------------------
			// pick a position to mutate in the middle of genes
